# Processing databases: IEDB, VDJdb, Mc-PAS for human
source("./vdj_curation.R")
source("./iedb_curation.R")
source("./McPAS_curation.R")


directory = "./Data/"


# 4) Concatenate all databases
...
#harmonization of Antigen_organism #organism_list <- data.frame(Antigen_organism = unique(all_database$Antigen_organism))
...
#harmonize Antigen #antigen_list <- data.frame(Antigen = unique(all_database$Antigen))
...
#Remove in the name antigen for some of these the prefix of epitope 
...
#save database
#write.table(all_database_final, paste0(directory, last_folder, "database_pooled_human_", format(Sys.time(), "%Y_%m_%d"), ".txt"), sep = "\t", quote = F, row.names = F)


#add filters
#same CDR3_alpha, CDR3_beta, Epitope, Antigen, Antigen_organism, PubMed_ID with lack of information in V_alpha, J_alpha, V_beta, J_beta, 
#Cell_subset => merge all informations with best Verified_score and Identification_score 
# 
# all_database_toCur <- all_database_human %>% 
#   group_by(CDR3_alpha, CDR3_beta, Epitope, Antigen, Antigen_organism, PubMed_ID) %>% 
#   mutate(doublon = n()) %>% filter(doublon >1)
# all_database_toCur <- all_database_toCur %>% 
#   mutate(seq_id = paste(CDR3_alpha, CDR3_beta, Epitope, Antigen, Antigen_organism, PubMed_ID, sep = "_"))
# 
# all_database_toCur_list <- split(all_database_toCur, f = all_database_toCur$seq_id)
# 
# #example: 
# test <- all_database_human %>% filter(PubMed_ID == "30418433", CDR3_alpha == "CAASASTDKLIF")
# 
# f <- function(x){
#   if(is.numeric(x))
#     max(x)
#   else if(is.na(x[-1]) && is.na(x[1]))
#     NA
#   else if(is.na(x[-1]) && !is.na(x[1]))
#     x[1]
#   else if(!is.na(x[-1]) && is.na(x[1]))
...
# for(i in names(all_database_toCur_list)){
#   print(i)
#   row_toKeep_df <- all_database_toCur_list[[i]] %>% as.data.frame()
#   if(n_distinct(unique(row_toKeep_df$Database))>1){#if data are in at least 2 databases: we merge informations contained in rows
#     row_toKeep_df <- row_toKeep_df %>% group_by(CDR3_alpha, CDR3_beta, Epitope, Antigen, Antigen_organism, PubMed_ID) %>%
#       summarise(across(everything(), f))
#   }
# 
#   row_toKeep_list[[i]] <- row_toKeep_df  
...
# 
# row_toKeep_largeDf <- rbindlist(row_toKeep_list)
# row_toKeep_largeDf <- row_toKeep_largeDf %>% select(-c(doublon, seq_id))
# 
# 
# all_database_single <- all_database %>% 
#   group_by(CDR3_alpha, CDR3_beta, Epitope, Antigen, Antigen_organism, PubMed_ID) %>% 
...
# all_database_single <- all_database_single %>% select(-c(doublon, row_id))
# 
# all_database_final_bis <- rbind(all_database_single, row_toKeep_largeDf)
